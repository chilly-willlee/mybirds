...
import { getRecentNearbyNotableObservations } from "@/lib/ebird/endpoints/observations";
import { CACHE_TTL, notableCacheKey } from "@/lib/ebird/cache";
...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = notableCacheKey(lat, lng, distKm, back);
  ...
  const observations = await getRecentNearbyNotableObservations(client, {
    lat, lng, dist: distKm, back,
  });

  await cache.set(cacheKey, observations, CACHE_TTL.notable);
